...
var uniqueRef = function uniqueRef(ref) {
...
function insertBefore(node, refNode, list) {
  ...
  if (refNode.nextSibling === node) {
    refNode.nextSibling = node.nextSibling;
  }
...
var BUBBLE_EVENTS_LIST = ['click', 'doubleClick', 'touch', 'longPress', 'pan'];

var isBubbleEvent = function isBubbleEvent(evType) {
  return evType && BUBBLE_EVENTS_LIST.indexOf(evType) !== -1;
};
...
Eventer.prototype.emitEvent = function emitEvent(type, event) {
  ...
  if (isEvObj && isBubble) {
    ...
    while (parent) {
      if (parent.events && parent.events.indexOf(type) !== -1) {
...
Eventer.prototype.send = function send(method, args, cb) {
  ...
  var __send = function __send(method, args, cb) {
    var cbId;
...
var Element = function (Node$$1) {
  function Element(type, opts, ref) {
    ...
    if (this.type == TextNodeType) {
      this.nodeType = NodeType.TEXT_NODE;
      this.text = opts.attr.value;
    } else {
    ...
    Object.defineProperty(this, 'events', {
      enumerable: true,
      get: function get() {
        return this.eventer && this.eventer.getEventList ? this.eventer.getEventList() : [];
      }
    });
  }
  ...
  Element.getCmp = function getCmp(cmpName) {
    return getComponent(cmpName);
  };
  ...
  Element.prototype.__initChildren = function __initChildren(children) {
    for (var i = 0; i < children.length; i++) {
      ...
      child.nodeType !== NodeType.COMMENT_NODE && this.realNodes.push(child);
    }
  };
  ...
  Element.prototype.getComputedStyle = function getComputedStyle() {
    var fromClass = Object.assign({}, this.staticClassStyle, this.classStyle);
    var fromStyle = Object.assign({}, this.staticStyle, this.style);
    return Object.assign({}, fromClass, fromStyle);
  };
  ...
  Element.prototype.updateEl = function updateEl(updates) {
    if (this.isNatived) {
      ...
      this.eventer && this.eventer.send(ACTION.UPDATE_ELEMENT, [this.ref, updates]);
    }
  };
  ...
  Element.prototype.addChild = function addChild(node, index) {
    if (this.isNatived) {
      ...
      this.eventer && this.eventer.send(ACTION.ADD_ELEMENT, [this.ref, node.toJSON(), index]);
      node.setInNative();
    }
  };
  ...
  Element.prototype.moveChild = function moveChild(ref, index) {
    if (this.isNatived) {
      ...
      this.eventer && this.eventer.send(ACTION.MOVE_ELEMENT, [this.ref, ref, index]);
    }
  };
  ...
  Element.prototype.appendChild = function appendChild(node, isImmediate) {
    ...
    if (node === this.children[this.children.length - 1]) {
      return node;
    }
    ...
    !node.ctx && node.__setCTX(this.ctx);
    return node;
  };
  ...
  Element.prototype.insertBefore = function insertBefore$$1(node, refNode, isImmediate) {
    ...
    var isRefComment = refNode.nodeType === NodeType.COMMENT_NODE;

    if (!isChild(this, refNode)) {
      console.error('reference node is not in this node');
      return;
    }
    ...
    if (isChild(this, node)) {
      originIndex = realNodelist.indexOf(node);
    }
    ...
    var nextRealNode = isRefComment ? refNode.nextRealNode() : refNode;
    var insertIndex = nextRealNode ? realNodelist.indexOf(nextRealNode) : realNodelist.length;

    if (originIndex === insertIndex) {
      needToMove = false;
    }

    insertIndex !== -1 && realNodelist.splice(insertIndex, 0, node);

    if (this.isNatived && needToMove) {
      if (node.isNatived) {
    ...
    if (isChild(this, node)) {
      if (node.nodeType !== NodeType.COMMENT_NODE) {
        if (this.isNatived && isImmediate) {
          ...
          this.eventer && this.eventer.send(ACTION.REMOVE_ELEMENT, [node.ref]);
          node.outNative();
        }
      }
  ...
  Element.prototype.setClass = function setClass(classList, isReplace) {
    var cls = Array.isArray(classList) ? classList : classList.split(' ');
...
var Comment = function (Node$$1) {
  ...
  Comment.prototype.nextRealNode = function nextRealNode() {
    ...
    while (node && node.nodeType === NodeType.COMMENT_NODE) {
...
Document.prototype.createElement = function createElement(type, opts) {
  ...
  var el = Cmp ? new Cmp(opts) : new Element(type, opts);
...
Document.prototype.createTextNode = function createTextNode(text, opts) {
  ...
  var type = 'text';
  opts.attr || (opts.attr = {});
  opts.attr.value = text;
  var TextNode = Element.getCmp(type);
  var textNode = TextNode ? new TextNode(opts) : new Element(type, opts);
...
Document.prototype.appendChild = function appendChild(body) {
  if (body instanceof Element) {
    ...
    this.render();
    oldBody && setTimeout(function () {
      return oldBody.destroy();
    });
  }
};
...
Document.prototype.render = function render(fnc) {
  ...
  this.tasker.sendTask([{
    module: MODULE.DOM,
    method: ACTION.CREATE_BODY,
    args: bodyDom.toJSON()
  }]);
  bodyDom.setInNative();
};
...
function registerModules(modules) {
  typeof modules === 'string' && (modules = JSON.parse(modules));
...
function initInjections(methods) {
  methods && (nativeMethods = nativeMethods.concat(methods));
...
Bridge.prototype.receive = function receive(id, args) {
  if (!id || isUndef(this.taskerMap[id])) {
    ...
    throw new Error("[receive from native]: 无效 instanceId: " + id + "\n        参数：" + _args + "\n      ");
  } else {
  ...
  if (args && Array.isArray(args) && args.length) {
    for (var i = 0, len = args.length; i < len; i++) {
      ...
      if (typeof param === 'function') {
        var cbId = this.genCallback(param);
        args[i] = cbId;
      }
    }
...
Tasker.prototype.actCallback = function actCallback(task) {
  ...
  if (args && (cb = this.callbackMap[cbId = args.shift()])) {
    cb.call(null, data);
  } else if (true) {
    throw new Error("Can't find callbackId: " + cbId);
  }
};
...
var PAGE_EVENTS = ['update', 'destroy'];
    ...
    Object.defineProperty(modules, method, {
      ...
      get: function get() {
        return function () {
          ...
          return this$1.tasker && this$1.tasker.moduleTask(name, method, args);
        };
      }
    });
  };

  for (var method in nativeModule) {
    loop(method);
  }
...
function freeze(obj, prop) {
  ...
  Object.freeze && Object.freeze(obj[prop]);
}

function freezeGlobalVar(globalVariables) {
  if (Array.isArray(globalVariables)) {
    globalVariables.forEach(function (globalVar) {
      freeze(Viola, globalVar);
    });
  }
}
...
function genComponent(type, fnc) {
  ...
  var C = function (type) {
    var C = function (Element$$1) {
      function C(opts, ref) {
        Element$$1.call(this, type, opts, ref);
      }
...
function VueScopeUp(exports, document) {
  ...
  function insertBefore(node, target, referenceNode) {
  ...
  function getStyle(classList, vnode, copy, inlineStyle, classStyle) {
    var stylesheet = vnode.context.$options._stylesheet;
    var res = {},
        extendFnc = copy ? mutiExtend(inlineStyle, classStyle, copy) : extend;
  ...
  var platformModules = [attrs, klass, events, style];
  ...
  var isReservedTag$1 = makeMap('template,script,style,element,content,slot,link,meta,svg,view,' + 'a,div,img,image,text,span,input,switch,textarea,spinner,select,p' + 'slider,slider-neighbor,indicator,canvas,' + 'list,cell,header,loading,loading-indicator,refresh,scrollable,scroller,' + 'video,web,embed,tabbar,tabheader,datepicker,timepicker,marquee,countdown', true);
  var canBeLeftOpenTag = makeMap('web,spinner,switch,video,textarea,canvas,' + 'indicator,marquee,countdown', true);
  var isRuntimeComponent = makeMap('batch,richtext,transition,transition-group', true);
...
function getFrameworkName(code) {
  var matchResult = pragma.exec(code);
  return matchResult ? matchResult[1] : '';
}

var defaultFW = [framework.name];
var fwMap = {};
fwMap[vanilla.name] = vanilla;
fwMap[framework.name] = framework;

function getFramework(code) {
  return fwMap[getFrameworkName(code)] || fwMap[defaultFW];
}
...
var globalMethodsName = Object.keys(globalMethods).concat(['callNative']);
...
function runCodeInCtx(ctx, code) {
  ...
  keys = keys.concat(globalMethodsName);
...
function init() {
  ...
  freezeGlobalVar(globalMethodsName.concat(['ViolaEnv']));
}
...
var esIndex = {
  init: function init$1() {
    _global.Viola = _global;
    init();
  },
  createInstance: createInstance,
  createInstanceCtx: createInstanceCtx,
  getFramework: getFramework,
  updateInstance: updateInstance,
  destroyInstance: destroyInstance$1,
  runCodeInCtx: runCodeInCtx,
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  'footer-refresh': ['refreshFinish'],
  'page-slider': ['setIndex', 'next', 'prev', 'exchange'],
  qrefresh: ['refresh', 'refreshFinish'],
  'kd-video': ['play', 'pause', 'stop', 'open', 'setVolume', 'getCurrentTime', 'isMuted', 'isEnded', 'isPaused', 'setMuted', 'isPlaying'],
  cell: ['animate'],
